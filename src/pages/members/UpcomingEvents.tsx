...
const UpcomingEventsPage = () => {
  ...
  const handleTagFilter = (tag: string) => {
    ...
    if (tag === "all") {
      ...
      setFilteredEvents((prev) =>
        prev.filter((event) =>
          event.tags.map((tag) => tag.toLowerCase()).includes(tag.toLowerCase())
        )
      );
    }
  };
  ...
  const handleEventRegister = async (event: Event) => {
    ...
    const toastId = toast.loading(
      `Registering for the ${event.title} event....`,
      {
        position: "top-center",
      }
    );
    try {
      ...
      toast.update(toastId, {
        render: `Registration for ${event.title} event was successful`,
        autoClose: 5000,
        isLoading: false,
        type: "success",
      });
      ...
      toast.update(toastId, {
        render:
          errMsg || "An error ocurred during the process, please try again",
        autoClose: 5000,
        isLoading: false,
        type: "error",
      });
  ...
  const fetchMyEvents = async () => {
    try {
      ...
      let allTags = data.reduce((acc: any, event: any) => {
        acc = [...acc, ...event.tags.map((tag: string) => tag.toLowerCase())];
        return acc;
      }, []);
          ...
          <div className="mb-6 flex flex-wrap gap-4">
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {tags.map((tag) => (
                  <option value={tag} className="capitalize">
                    {tag}
                  </option>
                ))}
              </Select>
                ...
                >
                  <div className="flex flex-col items-center">
                    ...
                    <div className="mt-4 flex gap-3">
                      ...
                      >
                        <HiOutlineCalendar className="mr-1 h-4" /> View Details
                      </Button>
                      <Button
                        size="sm"
                        color="blue"
                      ...
                      >
                        <HiOutlineLocationMarker className="mr-1 h-4" />{" "}
                        Register
                      </Button>
                    </div>
                  </div>
