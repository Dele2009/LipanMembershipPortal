  ...
  import { HiEye, HiMenu, HiPencil, HiTrash } from "react-icons/hi";
  ...
  export default function AdminEventsPage() {
    const {
      ...
      defaultValues: {
        // image: "",
        tags: [""],
        date: new Date().toISOString().slice(0, 16), // Default to current date
        allowed_memberships: [],
        is_featured: false,
      },
    });
                ...
                >
                  ...
                  <div className="absolute cursor-pointer top-2 right-2 p-2 rounded-md border border-white/30">
                    ...
                    >
                      <Dropdown.Item
                        as={Link}
                        to={`/admin/events/${event.title?.replace(" ", "-")}/${event.id}/users/`}
                      >
                        <HiEye className="mr-2 h-6 text-md text-blue-600" />{" "}
                        View User
                      </Dropdown.Item>
                      <Dropdown.Divider />
                      <Dropdown.Item
                        onClick={() => intializeAction(idx, "edit")}
                      >
