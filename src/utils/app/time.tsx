...
export const isTokenExpired = (token: string) => {
  ...
  const numberstodatestring = (num: number) => {
    const date = new Date(num * 1000); // Convert seconds to milliseconds
    return date.toISOString();
  };
  console.log("Token Expiration Date:", numberstodatestring(exp as number));
  console.log("Current Time:", numberstodatestring(currentTime));
  console.log((exp as number) < currentTime, exp, currentTime);
